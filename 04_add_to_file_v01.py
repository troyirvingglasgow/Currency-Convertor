...
class CurrencyConverterApp:
    ...
    def display_history(self):
        ...
        history_window.configure(bg="#bc6c25")

        # Create a Text widget to display history entries
        history_text = "\n".join(self.history)
        history_text_widget = tk.Text(history_window, padx=10, pady=10, fg="#361d18",
                                      font=("Arial", 12), bg="#fefae0", wrap="word")
        history_text_widget.insert(tk.END, history_text)
        history_text_widget.config(state=tk.DISABLED)
        history_text_widget.pack(expand=True, fill=tk.BOTH)

        # Add a button to add history to a text file
        add_to_file_button = tk.Button(history_window, text="Add to File", command=self.add_to_file,
                                       bg=BUTTON_COLOR, fg="#361d18", font=("Arial", 12, "bold"))
        add_to_file_button.pack(side=tk.BOTTOM, padx=20, pady=(0, 10), fill=tk.X)

        # Add a back button
        back_button = tk.Button(history_window, text="Back", command=lambda: self.close_history(history_window),
                                bg="#fefae0", fg="#361d18", font=("Arial", 12, "bold"))
        back_button.pack(side=tk.BOTTOM, padx=20, pady=10, fill=tk.X)

    def add_to_file(self):
        with open("conversion_history.txt", "a") as file:
            for entry in self.history:
                file.write(entry + "\n")
        messagebox.showinfo("File Updated", "Conversion history has been added to the file.")

    def close_history(self, history_window):
